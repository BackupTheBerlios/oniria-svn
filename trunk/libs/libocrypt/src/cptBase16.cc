...
size_t cptBase16::decode(QByteArray * dst, const char * src, size_t len, bool lc)
{
	size_t d_sz;
...
size_t cptBase16::decode(QByteArray * dst, const QByteArray * src, bool lc)
{
	return cptBase16::decode(dst, src->data(), src->size(), lc);
}

size_t cptBase16::decode(QByteArray * dst, const QString& src, bool lc)
{
	QByteArray tmp = src.toLocal8Bit();
	return cptBase16::decode(dst, &tmp, lc);
}

QString cptBase16::encode(const uint8_t * src, size_t len, bool lc)
{
	QString enc;
	size_t enc_len, i, j;
	const char * b16_table;
...
QString cptBase16::encode(const char * src, size_t len, bool lc)
{
	return encode(reinterpret_cast<const char *>(src), len, lc);
}

QString cptBase16::encode(const QString& src, bool lc)
{
	QByteArray tmp = src.toLocal8Bit();
	return encode(&tmp, lc);
}

QString cptBase16::encode(QByteArray * src, bool lc)
{
