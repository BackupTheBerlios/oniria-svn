...
size_t cptBase64::decode(QByteArray * dst, const char * src, size_t len)
{
	size_t d_sz;
	if (len == 0)
		len = strlen(src);
...
size_t cptBase64::decode(QByteArray * dst, const QByteArray * src)
{
	return cptBase64::decode(dst, src->data(), src->size());
}

size_t cptBase64::decode(QByteArray * dst, const QString& src)
{
	QByteArray tmp = src.toLocal8Bit();
	return cptBase64::decode(dst, &tmp);
}

QString cptBase64::encode(const uint8_t * src, size_t len)
{
	QString enc;
	size_t enc_len, i, j;
...
QString cptBase64::encode(const char * src, size_t len)
{
	return encode(reinterpret_cast<const uint8_t *>(src), len);
}

QString cptBase64::encode(const QString& src)
{
	QByteArray tmp = src.toLocal8Bit();
	return encode(&tmp);
}

QString cptBase64::encode(QByteArray * src)
{
